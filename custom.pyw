...
#     window = customtkinter.CTk()
#     window.title("Instruction for cryptorgraphy")  
...
# frame_4_button = customtkinter.CTkButton(navigation_frame, corner_radius=0, height=40, border_spacing=10, text="SSH",
#                                                       fg_color="transparent", text_color=("gray10", "gray90"), hover_color=("gray70", "gray30"),
#                                                       anchor="w", command=frame_3_button_event)
# frame_4_button.grid(row=4, column=0, sticky="new")

# frame_5_button = customtkinter.CTkButton(navigation_frame, text="Instruction", command=frame_4_button_event)
# frame_5_button.grid(row=5, column=0, sticky="new")

def encrypting():
    textbox2.delete("0.0", "end")
    txt = textbox1.get("1.0", END)
    shift = int(shifting.get())
    enc = ces.encryption(txt, shift)
...
def open_picture():
    try:
        ...
        picture_label.grid(row=1, column=0, padx=(20, 20), pady=(20, 20))
        picture_entry.grid(sticky="nsew")
        enc.grid(row=2, column=1, padx=(10, 0), pady=(0, 0), sticky="s")
        return pic
    except BaseException:
        CTkMessagebox(title="Error", message="This type of file is not allowed!!!", icon="cancel")
...
def encrypt_picture():
    ...
    if enc == "":
        CTkMessagebox(title="Info", message="Enter your message!!!")
    else:
        CTkMessagebox(title="Info", message="file save with name 'newimage.png'" , icon="check")
        crypto_picture.stega_encrypt(pic, enc)
        textbox2 = customtkinter.CTkTextbox(tabview.tab("Encrypt"), width=2500, height=250)
        textbox2.grid(column=0, row=3, padx=(20, 20), pady=(10, 0), sticky="nsew")
        a = []
        with open("keys.txt", "r") as f:
            for i in f:
                a.append(i)
            textbox2.insert("0.0", "".join(a))

def decrypt_picture():
    picture = customtkinter.filedialog.askopenfilename()
    pic = picture
    # picture_image = customtkinter.CTkImage(Image.open(os.path.join(image_path, picture)), size=(600, 300))
    # picture_label = customtkinter.CTkLabel(tabview.tab("Decrypt"), image=picture_image, text="")
    # picture_label.grid(row=1, column=0, padx=(20, 20), pady=(20, 20))
    keygen = textbox_second.get("1.0", END)
    with open("keys.txt", "w") as f:
        f.write(keygen)
    textbox2 = customtkinter.CTkTextbox(tabview.tab("Decrypt"), width=2500, height=250)
    textbox2.grid(column=0, row=3, padx=(20, 20), pady=(10, 0), sticky="nsew")
    textbox2.insert("0.0", crypto_picture.stega_decrypt(pic))

def decrypting():
    textbox2.delete("0.0", "end")
    txt = textbox1.get("1.0", END)
    shift = int(shifting.get())
    enc = ces.decryption(txt, shift)
    textbox2.insert("0.0", enc)
...
shifting = customtkinter.CTkComboBox(label1, values=["1", "2", "3", "4", "5"])
shifting.grid(row=0, column=2, padx=20, sticky="e")
...
decrypt_pic = customtkinter.CTkButton(tabview.tab("Decrypt"), text="Decrypt image", fg_color="#2F4F4F", hover_color="#696969", width=50, height=30, corner_radius=0, command=decrypt_picture)
decrypt_pic.grid(row=0, column=0, padx=(10, 0), pady=(0, 0), sticky="n")

textbox_second = customtkinter.CTkTextbox(tabview.tab("Decrypt"), width=2500, height=250)
textbox_second.grid(column=0, row=2, padx=(20, 20), pady=(10, 0), sticky="nsew")
textbox_second.insert("0.0", "write keys")

picture_label = customtkinter.CTkLabel(tabview.tab("Encrypt"), text="")
picture_label.grid(row=1, column=0)
